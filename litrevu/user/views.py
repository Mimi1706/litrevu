...
def find_user(request):
    ...
    try:
        ...
        if user_to_follow == request.user:
            return follow(request, error_message="Vous ne pouvez pas vous abonner à vous-même.")

        # Check if already following
        elif UserFollows.objects.filter(user=request.user, followed_user=user_to_follow).exists():
            return follow(request, error_message="Vous êtes déjà abonné à cet utilisateur")
        ...
        return follow(request, error_message=f"Aucun utilisateur du nom de '{username}' n'a été trouvé.")
